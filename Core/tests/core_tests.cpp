...
public:
    ...
    ~EnvironmentVariableGuard() {
        if (original_) set(*original_); else unset();
    }
...
std::filesystem::path unique_fixture(const std::string& prefix) {
    return std::filesystem::temp_directory_path() / (prefix + std::to_string(
...
const herdme::RuntimeCheck* runtime_named(
    const std::vector<herdme::RuntimeCheck>& runtimes,
...
int main() {
    ...
    expect(
        extension_json.find("\"missing\":[\"mbstring\"]") != std::string::npos
    ...
    const auto php_module_fixtures = std::filesystem::path(__FILE__).parent_path()
        / "fixtures" / "php-modules";
    const std::vector<std::string> php_module_fixture_names = {
        "complete", "missing-mbstring", "missing-curl-and-xml"
    };
    for (const auto& name : php_module_fixture_names) {
        ...
        expect(
            report.missing == nonempty_lines(read_file(expected_path)),
    ...
    expect(wordpress_site != nullptr && wordpress_site->framework == "WordPress", "wp-config.php must identify WordPress");
    ...
    expect(
        laravel_site != nullptr && laravel_site->php_version == std::optional<std::string>{"8.3"},
        "site scanning must trim managed PHP overrides"
    );
    expect(
        laravel_site != nullptr && laravel_site->node_version == std::optional<std::string>{"22"},
    ...
    const herdme::Site escaped_site{
        "demo\"\nsite",
        std::filesystem::path("folder") / "quoted\"path",
        "Laravel",
        true,
        "8.4",
        std::nullopt
    ...
    const std::vector<herdme::RuntimeCheck> doctor_checks = {
        {"php", std::filesystem::path("bin") / "php\"tool", "system", true},
        {"node", std::nullopt, "missing", false}
    };
    ...
    expect(
        php_runtime != nullptr && php_runtime->usable && php_runtime->source == "managed",
        "runtime inspection must identify managed PHP"
    );
    expect(
        node_runtime != nullptr && node_runtime->usable && node_runtime->source == "managed",
        "runtime inspection must identify managed Node.js"
    );
    expect(
        composer_runtime != nullptr && !composer_runtime->usable && composer_runtime->source == "missing",
