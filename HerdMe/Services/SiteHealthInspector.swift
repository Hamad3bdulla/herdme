...
enum SiteHealthInspector {
    ...
    static func inspect(
        ...
        if isLaravel {
            ...
            checks.append(check("app-key", "Laravel application key", !(appKey?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ?? true), "APP_KEY is missing.", true))
            let vendor = path.appendingPathComponent("vendor/autoload.php")
            checks.append(check("composer", "Composer dependencies", fileManager.isReadableFile(atPath: vendor.path), "Run composer install.", true))
            let storage = [
            ...
            let writable = storage.allSatisfy { directory in
                fileManager.isWritableFile(atPath: directory.path) || fileManager.isWritableFile(atPath: directory.deletingLastPathComponent().path)
            }
            ...
            checks.append(check("storage-link", "Storage link", fileManager.fileExists(atPath: storageLink.path), "Run artisan storage:link.", true))
